    ...
    .then((response) => {
      ...
      databases.getDocument(DATABASE_ID, COLLECTION_ID, userId)
        .then((doc) => {
          ...
          document.getElementById("reg-number").value = doc.RegistrationNumber || "";
          ...
          document.getElementById("income").value = doc.FamilyAnnualIncome || "";
        })
        .catch((error) => {
...
function uploadAadhar(userId, applicationData, aadharFile) {
  ...
  storage.createFile(bucketId, userId, aadharFile)
    .then((fileResponse) => {
    ...
    .catch((error) => {
      ...
      document.getElementById("message").textContent = "Aadhar Upload Failed. Please try again.";
    });
}
...
function saveApplicationData(userId, applicationData) {
  databases.createDocument(DATABASE_ID, NEW_COLLECTION_ID, userId, applicationData)
    .then((response) => {
    ...
    .catch((error) => {
      ...
      document.getElementById("message").textContent = "Failed to submit application. Please try again.";
    });
}
